...
A helper function to send payload across websocket clusters.
This function will send payload using existing websocket connection instance if available.
...
### `lightwsd.fns.broadcast([payload], [options])`

- `payload` (optional any, default: `{}`) The payload to send to websocket connection.
- `options` (optional object, default: `{}`) The options passed to `connection.send` function of connection instance of ws package.

A helper function to send payload to all active websockets.
This function will find all active websockets and send broadcast signal to every possible sockets.
Also, will return promise and end it when all try to send signal via redis are done.
If you're handling a lot of servers that may performance can be degraded by publishing messages a lot of channels, you need to clean unused values before publishing.
Every keys generated by lightwsd starts with `wsd:` and namespace follows.
However, by default, the expire rate of redis store is 3600, so you don't need to care about it at the time.
